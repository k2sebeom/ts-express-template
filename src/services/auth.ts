...
export default class AuthService {
    ...
    public createUser = async (email: string, password: string): Promise<Auth> => {
    ...
    public isPasswordValid = async (pw: string, pwHash: string): Promise<boolean> => {
        return await bcrypt.compare(pw, pwHash);
    }

    public isRefreshTokenValid = async (refreshToken: string, id: number): Promise<boolean> => {
        const user = await db.auth.findUnique({
            where: { id }
        });
